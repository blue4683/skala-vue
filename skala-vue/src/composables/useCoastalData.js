 ...
 * 격자 dedupe → 병렬 조회 → 구간별 시각 인덱스 접근 함수를 제공한다.
 */
export function useCoastalData() {
  ...
  async function load() {
    ...
    try {
      ...
      // 같은 격자에 속한 구간이 여러 개일 수 있다 → 중복 제거 후 한 번만 조회
      const uniqueGrids = [
      ...
      // 실제 API 사용 시:
      // const results = await Promise.all(uniqueGrids.map(g => fetchVillageForecast(g.nx, g.ny)))
    } catch (e) {
